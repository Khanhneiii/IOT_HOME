...
#define DHT_URL "https://dht-home-b0439-default-rtdb.asia-southeast1.firebasedatabase.app/dht11.json"
...
void dht11_read_task(void *pvPara) {
    int pin = (int)pvPara;
    ...
    while (1) {
        ...
        cJSON_AddNumberToObject(dht,"Temperature",temp);
        cJSON_AddNumberToObject(dht,"Humidity",humi);
        char *dht_data = cJSON_PrintUnformatted(dht);

        ESP_LOGI("DHT","Temp: %d", temp);

        ESP_LOGI("DHT","Hemi: %d", humi);

        http_post_FB(DHT_URL,dht_data);
        cJSON_free(dht);
        free(dht_data);
